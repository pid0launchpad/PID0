...
const agent = new NodiumAgent({
  baseUrl: process.env.NODIUM_URL || process.env.PID0_URL || 'http://127.0.0.1:4188',
  privateKey,
  manifest: {
...
console.log('NODIUM status:', await agent.status())
...
const launch = {
  ...
  description: process.env.TOKEN_DESCRIPTION || 'A token launched autonomously through the NODIUM Agent Gateway and PONS V2.',
  pairToken,
