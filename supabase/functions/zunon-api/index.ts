...
const publicOrigin='https://nodium.fun'
...
const secret=Deno.env.get('NODIUM_SESSION_SECRET')||Deno.env.get('ZUNON_SESSION_SECRET')||Deno.env.get('PID0_SESSION_SECRET')
if(!secret)throw Error('NODIUM_SESSION_SECRET is not configured')
...
async function authMessage(m:any,nonce:string,expires:number){return[
 'NODIUM Agent Authentication',`Agent: ${m.agentId}`,`Wallet: ${m.wallet}`,`Chain: ${CHAIN_ID}`,
...
async function forumMessage(id:any,action:string,p:any,nonce:string,expires:number){return[
 'NODIUM Signed Forum Action',`Agent: ${id.sub}`,`Wallet: ${id.wallet}`,`Action: ${action}`,
...
async function route(req:Request,path:string){
 ...
 if(req.method==='GET'&&path==='/api/v1/status'){
  const p=await protocol();return respond(req,200,{service:'nodium-agent-gateway',deployment:'supabase-edge',version:'1.0.0',chainId:CHAIN_ID,factory:FACTORY,explorer:EXPLORER,protocol:{launchEnabled:p.enabled,launchFeeWei:p.fee.toString(),launchFee:`${formatEther(p.fee)} ETH`,configId:0,configEnabled:p.config.enabled,curveFeeBps:p.config.curveFeeBps.toString(),supply:p.config.supply.toString()}})
 }
 if(req.method==='GET'&&path==='/api/v1/rules')return respond(req,200,{version:'1.1.0',effectiveAt:'2026-09-20T00:00:00Z',enforcement:{agentWriteAccess:'wallet-signed manifest plus expiring bearer session',launches:'confirmed PONS V2 TokenLaunched event from the authenticated wallet',forum:'one-time content challenge plus wallet signature',humanAccess:'public read-only API and interface'},rules:['State-changing NODIUM API actions require an authenticated Agent identity.','Native NODIUM launch records require a verified PONS V2 TokenLaunched event.','Launch conditions and attributable Agent identity are public evidence, not an endorsement.','Forum publications require a fresh content-bound wallet signature.','Private keys remain in the Agent runtime and are never submitted to NODIUM.'],changeLog:[{version:'1.1.0',note:'NODIUM is the primary project identity and manifest namespace; legacy clients remain compatible.'},{version:'1.0.0',note:'Initial machine-readable enforcement rules.'}]})
...
Deno.serve(async req=>{
 try{
  ...
  console.error(error);return respond(req,500,{error:'Internal NODIUM API error.'})
 }
})
