...
const PORT = Number(process.env.NODIUM_PORT || process.env.PID0_PORT || 4188)
const HOST = process.env.NODIUM_HOST || process.env.PID0_HOST || '127.0.0.1'
const DB_PATH = process.env.NODIUM_DB_PATH || process.env.PID0_DB_PATH || join(dataRoot, 'zunon.sqlite')
const KEY_PATH = join(dataRoot, 'session.key')
const SESSION_SECONDS = Number(process.env.NODIUM_SESSION_SECONDS || process.env.PID0_SESSION_SECONDS || 900)
const CHALLENGE_SECONDS = Number(process.env.NODIUM_CHALLENGE_SECONDS || process.env.PID0_CHALLENGE_SECONDS || 300)
const BODY_LIMIT = 64 * 1024

function loadSecret() {
  if (process.env.NODIUM_SESSION_SECRET || process.env.PID0_SESSION_SECRET) return Buffer.from(process.env.NODIUM_SESSION_SECRET || process.env.PID0_SESSION_SECRET)
...
function json(res, status, value, extra = {}) {
  ...
  res.writeHead(status, {
    ...
    'access-control-allow-origin': process.env.NODIUM_ALLOWED_ORIGIN || process.env.PID0_ALLOWED_ORIGIN || '*',
...
async function api(req, res, pathname) {
  ...
  if (req.method === 'GET' && pathname === '/api/v1/status') {
    ...
    return json(res, 200, {
      service: 'nodium-agent-gateway', version: '1.0.0', chainId: CHAIN_ID,
  ...
  if (req.method === 'GET' && pathname === '/api/v1/rules') {
    return json(res, 200, {
      ...
      rules: [
        'State-changing NODIUM API actions require an authenticated Agent identity.',
        'Native NODIUM launch records require a verified PONS V2 TokenLaunched event.',
        ...
        'Private keys remain in the Agent runtime and are never submitted to NODIUM.',
      ],
      changeLog: [{ version: '1.1.0', note: 'NODIUM becomes the primary project identity and manifest namespace; legacy clients remain compatible.' }, { version: '1.0.0', note: 'Initial machine-readable enforcement rules.' }],
    })
  }
...
server.listen(PORT, HOST, () => {
  console.log(`NODIUM Agent Gateway listening on http://${HOST}:${PORT}`)
