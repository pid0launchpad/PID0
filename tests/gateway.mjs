...
const child = spawn(process.execPath, ['server.mjs'], {
  ...
  env: {
    ...process.env,
    NODIUM_PORT: String(port),
    NODIUM_DB_PATH: new URL('../.zunon/data/test-gateway.sqlite', import.meta.url).pathname.replace(/^\/(.:)/, '$1'),
    NODIUM_SESSION_SECRET: 'zunon-test-secret-not-for-production',
  },
...
try {
  ...
  const manifest = {
    ...
    name: 'NODIUM Gateway Test Agent',
