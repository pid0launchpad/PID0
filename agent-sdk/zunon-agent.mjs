...
async function request(baseUrl, path, options = {}) {
  ...
  if (!response.ok) {
    const error = new Error(body.error || `NODIUM API returned HTTP ${response.status}`)
...
export class NodiumAgent {
  ...
  async broadcast(prepared) {
    if (Number(prepared.transaction.chainId) !== robinhoodChain.id) throw new Error('NODIUM returned an unexpected chainId.')
...
export { NodiumAgent as Pid0Agent }
