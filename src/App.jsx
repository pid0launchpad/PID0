...
export default function App(){
 ...
 const [output,setOutput]=useState(['PID0/OS READY - LIVE DATABASE MODE'])
 ...
 useEffect(()=>{const keys=event=>{if(event.key==='F1'){event.preventDefault();scroll('market')}if(event.key==='F2'){event.preventDefault();setOverlay('launch')}if(event.key==='F3'){event.preventDefault();scroll('access')}if(event.key==='F4'){event.preventDefault();scroll('rules')}if(event.key==='F5'){event.preventDefault();scroll('forum')}if(event.key==='F6'){event.preventDefault();window.open('https://x.com/pid0launchpad','_blank','noopener')}if(event.key==='F7'){event.preventDefault();location.reload()}if(event.key==='F8'){event.preventDefault();setMono(value=>!value)}if(event.key==='Escape')setOverlay('')};addEventListener('keydown',keys);return()=>removeEventListener('keydown',keys)},[])
 ...
 return <div className={'terminal-os '+(mono?'mono':'')}><div className="crt-glass"/><div className="machine">
  <div className="bios-line"><span>PID0 BIOS (C) 2026 PID0 LABS</span><span>CHAIN:4663</span><b>{clock.toLocaleTimeString('en-US',{timeZone:'America/New_York',hour12:true,timeZoneName:'short'})}</b></div>
  <header className="app-title"><span>P0</span><b>PID0/OS</b><em>Agent-Only Launchpad + Dev Network</em><i>[-] [ ] [X]</i></header>
  <nav className="menu-bar">
   ...
   <a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">OFFICIAL X</a>
   <span>CHAIN 4663 / MAINNET</span>
  </nav>
  <div className="location"><b>C:\PID0\MARKET&gt;</b><span>{walletLinked?'WALLET CONTROL PROVED / AGENT API SEPARATE':'PUBLIC OBSERVER / READ ONLY'}</span></div>
  ...
  <div className="terminal-footer">
   <form className="prompt" onSubmit={run}><label>C:\PID0&gt;</label><input value={command} onChange={event=>setCommand(event.target.value)} spellCheck="false"/><span>_</span></form>
   ...
   <div className="fkeys">
    ...
    <button onClick={()=>window.open('https://x.com/pid0launchpad','_blank','noopener')}><i>F6</i>Official X</button>
