...
const fixed=[
 {id:'pid0',token:'P0 AGENT',ticker:'P0',type:'OFFICIAL PROJECT TOKEN',status:'PONS CONFIRMED',agent:'PID0 NETWORK',network:'ROBINHOOD CHAIN / 4663',contract:'0x8c6fbb8483a9a4cf6b522e121e7759528c966a42',curve:'0x4c8e64b5dCAE225E6b4c6c24DBbAc50953823c2D',pairToken:ZERO_ADDRESS,pairLabel:'NATIVE ETH',txHash:'0x13c438475631748f5b6dbfddf90d64573774f48ff6626bd56db37fc3e425941d',launchBlock:'67435487',description:'$P0 is the official token of the PID0 Agent launchpad and development network. Its contract, PONS V2 curve, deployer, native-ETH pair and TokenLaunched event are verified on Robinhood Chain.',proof:'OFFICIAL PONS TOKENLAUNCHED EVENT VERIFIED'},
...
export default function VerifiedMarket(){
 ...
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: PID0" meta={Math.max(0,records.length-3)+' AGENT / 2 TEST VERIFIED / 1 OFFICIAL VERIFIED'} className="list-pane token-list verified-registry">
   ...
   <div className="market-summary verified-summary"><span>VERIFIED RECORDS <b>{String(records.length).padStart(2,'0')}</b></span><span>$P0 OFFICIAL TOKEN <b>PONS CONFIRMED</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">OFFICIAL $P0 + TEST 01 + TEST 02 / ONCHAIN VERIFIED</div>
  </Pane>
  ...
  <Pane title="PROJECT :: PID0" meta={sync} className="project-pane pid0-profile pid0-plain verified-project">
   <div className="pid0-scroll">
    ...
    <div className="pid0-token-card">
     <div className="pid0-token-card-head"><b>$P0 TOKEN</b><span>PONS CONFIRMED</span></div>
     ...
     <p>The official $P0 address above is backed by a PONS V2 TokenLaunched event on Robinhood Chain. The event verifies its curve, deployer, native-ETH pair and launch configuration.</p>
    </div>
    ...
    <Section title="REGISTRY COVERAGE"><p>The middle panel combines the onchain-verified official $P0 and test records with confirmed Agent launches stored by the PID0 Gateway. It does not claim to index every token ever launched through PONS. External PONS launches need a future historical indexer.</p><p>Price, liquidity and holders remain NOT INDEXED until real market-data sources are connected. PID0 does not manufacture these numbers.</p></Section>
    ...
    <Section title="LAUNCH ORDER AND STATUS OF $P0"><p>TEST 01 and TEST 02 were used to verify the Agent-to-PONS launch path. They remain visibly classified as test launches and are not the official project token.</p><p>$P0 is now deployed through PONS V2 on Robinhood Chain. Its contract reports the name P0 AGENT, symbol P0, 18 decimals and a total supply of 1,000,000,000 P0. The Registry treats the factory event and chain state as the authoritative launch evidence.</p></Section>
    ...
    <div className="project-section final-note"><b>OFFICIAL IDENTIFIERS</b><p>Robinhood Chain mainnet / Chain ID 4663. $P0: {pid0Token.contract}. PONS V2 factory: {FACTORY}.</p><p><a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">[OFFICIAL X]</a> <a href={apiUrl('/api/v1/status')} target="_blank" rel="noreferrer">[LIVE STATUS]</a> <a href={apiUrl('/api/v1/rules')} target="_blank" rel="noreferrer">[ACTIVE RULES]</a></p></div>
   </div>
  </Pane>
 </main>
}
