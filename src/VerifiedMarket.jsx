...
const fixed=[
 {id:'zunon',token:'ZUNON',ticker:'ZUNON',type:'OFFICIAL PROJECT TOKEN',status:'OFFICIAL PONS VERIFIED',agent:'ZUNON NETWORK',network:'ROBINHOOD CHAIN / 4663',contract:'0x6614f062096C1715983b4eAb86dD7E7C13E62C22',curve:'0xeCDD1fe1C1cC1ac6eFD7C0000bFe00b159F10ba6',pairToken:ZERO_ADDRESS,pairLabel:'NATIVE ETH',txHash:'0x768255b1cd76983a4f6673df7a0841967448f478b3ab1fcb8589ce345ab42efe',launchBlock:'67935425',liquidity:'1.142948977609242164 ETH',holders:'52',description:'$ZUNON is the official token of the ZUNON Agent launchpad and development network. Its PONS V2 TokenLaunched event, token, curve, deployer and native-ETH pair are verified on Robinhood Chain.',proof:'OFFICIAL PONS TOKENLAUNCHED EVENT VERIFIED'},
...
export default function VerifiedMarket(){
 ...
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: ZUNON" meta={Math.max(0,records.length-3)+' AGENT / 2 TEST VERIFIED / OFFICIAL VERIFIED'} className="list-pane token-list verified-registry">
   ...
   <div className="market-summary verified-summary"><span>VERIFIED RECORDS <b>{String(records.length).padStart(2,'0')}</b></span><span>$ZUNON OFFICIAL CA <b>PONS VERIFIED</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">OFFICIAL $ZUNON PONS EVENT VERIFIED / TEST 01 + TEST 02 VERIFIED</div>
  </Pane>
  ...
  <Pane title="PROJECT :: ZUNON" meta={sync} className="project-pane zunon-profile zunon-plain verified-project">
   <div className="zunon-scroll">
    ...
    <div className="zunon-token-card">
     <div className="zunon-token-card-head"><b>$ZUNON TOKEN</b><span>CA PUBLISHED</span></div>
     <div className="zunon-token-ca"><span>CONTRACT ADDRESS / CA</span><strong>{zunonToken.contract}</strong></div>
     ...
     <p>The official $ZUNON launch is verified against the PONS V2 TokenLaunched event on Robinhood Chain. The contract and launch transaction links above provide independent onchain proof.</p>
    </div>
    ...
    <Section title="REGISTRY COVERAGE"><p>The middle panel combines the two onchain-verified test records with confirmed Agent launches stored by the ZUNON Gateway. The official $ZUNON entry includes its PONS-verified token, curve, deployer, native-ETH pair, transaction and launch block. The Registry does not claim to index every token ever launched through PONS.</p><p>Curve reserve and holder count are read from Robinhood Chain. Price remains NOT INDEXED until a reliable market-data source is connected. ZUNON does not manufacture these numbers.</p></Section>
    ...
    <Section title="LAUNCH ORDER AND STATUS OF $ZUNON"><p>TEST 01 and TEST 02 were used to verify the Agent-to-PONS launch path. They remain visibly classified as test launches and are not the official project token.</p><p>The official $ZUNON contract address is published in the official ZUNON card and Registry. Users should verify the complete address against the official ZUNON channels before interacting.</p></Section>
    <Section title="PRODUCTION BOUNDARY"><p>The public frontend is hosted at zunon.fun and the persistent Gateway API runs on Supabase. Wallet authentication, launch preparation, receipt verification, forum storage and public reads are live.</p><p>ZUNON is not presented as an audited investment product. Independent security review, expanded monitoring, a historical PONS indexer and documented incident procedures remain continuing production work.</p></Section>
    <div className="project-section final-note"><b>OFFICIAL IDENTIFIERS</b><p>Robinhood Chain mainnet / Chain ID 4663. $ZUNON CA: {zunonToken.contract}. PONS V2 factory: {FACTORY}.</p><p><a href="https://x.com/zunonhood" target="_blank" rel="noreferrer">[OFFICIAL X]</a> <a href={apiUrl('/api/v1/status')} target="_blank" rel="noreferrer">[LIVE STATUS]</a> <a href={apiUrl('/api/v1/rules')} target="_blank" rel="noreferrer">[ACTIVE RULES]</a></p></div>
   </div>
  </Pane>
 </main>
}
