...
const official = {
  ...
  status: 'OFFICIAL VERIFIED', agent: 'NODIUM NETWORK', network: 'SOLANA MAINNET-BETA',
  contract: 'AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump', curve: 'A4dQ1QccT4GLaXyB3Bgfy6HrGk3UW7nJbHc67ZtFhmuV', txHash: '', pairLabel: 'SOL',
  description: 'The official NODIUM Token-2022 mint and its Pump bonding curve are verified on Solana mainnet.',
  proof: 'OFFICIAL PUMP BONDING CURVE VERIFIED',
}
...
export default function VerifiedMarket() {
  ...
  return <main className="main-panes verified-market">
    <Pane title={'INSPECT :: $' + selected.ticker} meta={selected.status} className="inspect-pane project-inspect verified-inspect">
      ...
      {selected.txHash ? <a className="verified-action" href={`${EXPLORER_URL}/tx/${selected.txHash}`} target="_blank" rel="noreferrer">[OPEN SOLANA TRANSACTION]</a> : <button className="execute" disabled>{selected.id === 'nodium' ? '[OFFICIAL TOKEN VERIFIED]' : '[TEST TOKEN VERIFIED]'}</button>}
    </Pane>

    <Pane title="TOKEN REGISTRY :: SOLANA" meta={`${launches.length} PUMP LAUNCH${launches.length === 1 ? '' : 'ES'} / 2 TEST VERIFIED / OFFICIAL CA VERIFIED`} className="list-pane token-list verified-registry">
      ...
      <div className="market-summary verified-summary"><span>LOCAL PUMP RECORDS <b>{String(launches.length).padStart(2, '0')}</b></span><span>$NODIUM OFFICIAL MINT <b>VERIFIED</b></span><span>TEST TOKENS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
      <div className="pane-foot">OFFICIAL $NODIUM + TEST 1 + TEST 2 PUMP VERIFIED</div>
    </Pane>
    ...
    <Pane title="PROJECT :: NODIUM" meta={sync} className="project-pane nodium-profile nodium-plain verified-project">
      <div className="nodium-scroll">
        ...
        <div className="nodium-token-card">
          <div className="nodium-token-card-head"><b>$NODIUM TOKEN</b><span>CA PUBLISHED</span></div>
          <div className="nodium-token-ca"><span>OFFICIAL TOKEN MINT</span><strong>AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump</strong></div>
          <div className="nodium-token-links"><a href="https://x.com/nodiumdotfun" target="_blank" rel="noreferrer">[OFFICIAL X]</a><a href="https://solscan.io/token/AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump" target="_blank" rel="noreferrer">[SOLSCAN]</a><a href="https://pump.fun/coin/AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump" target="_blank" rel="noreferrer">[PUMP.FUN]</a></div>
          <p>The official NODIUM mint is published and verified against its Pump bonding curve on Solana. Always compare the complete address before interacting.</p>
        </div>
        ...
        <div className="nodium-summary"><b>LIVE SYSTEM STATE</b><span>RPC: {sync}</span><span>PROGRAM: {short(PROGRAM_ID)}</span><span>PUMP CREATE: ENABLED</span><span>QUOTE: SOL</span><span>LOCAL LAUNCHES: {launches.length}</span><span>OFFICIAL MINT: VERIFIED</span><span>TEST TOKENS: 2 VERIFIED</span></div>
        ...
        <Section title="OFFICIAL NODIUM STATUS"><p>The official $NODIUM mint is AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump. Its Token-2022 mint and Pump bonding curve are verified on Solana mainnet.</p></Section>
        <div className="project-section final-note"><b>OFFICIAL IDENTIFIERS</b><p>Network: Solana mainnet-beta. Pump program: {PROGRAM_ID}. $NODIUM mint: AC3xPKNjmhZpaNfuSymPUsHtkud1jmcn3ZAAUe9vpump.</p><p><a href="https://x.com/nodiumdotfun" target="_blank" rel="noreferrer">[OFFICIAL X]</a> <a href="https://pump.fun" target="_blank" rel="noreferrer">[PUMP.FUN]</a></p></div>
      </div>
    </Pane>
  </main>
}
