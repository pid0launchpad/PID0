...
const fixed=[
 {id:'pid0',token:'PID0',ticker:'PID0',type:'OFFICIAL PROJECT TOKEN',status:'PRE-LAUNCH',agent:'PID0 NETWORK',network:'ROBINHOOD CHAIN / 4663',description:'$PID0 is the official token of the entire PID0 project. Its launch is intentionally pending while TEST 01 and TEST 02 are used to validate the real Agent-to-PONS launch process first.',proof:'OFFICIAL LAUNCH PENDING TEST COMPLETION'},
...
export default function VerifiedMarket(){
 ...
 const pid0Launch=launches.find(item=>String(item.request?.symbol||'').toUpperCase()==='PID0')
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: PID0" meta={launches.length+' AGENT / 2 TEST VERIFIED / 1 OFFICIAL'} className="list-pane token-list verified-registry">
   ...
   <div className="market-summary verified-summary"><span>AGENT RECORDS <b>{String(launches.length+2).padStart(2,'0')}</b></span><span>$PID0 OFFICIAL TOKEN <b>PRE-LAUNCH</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">TEST FLOW FIRST / OFFICIAL $PID0 LAUNCH AFTER VERIFICATION</div>
  </Pane>
  ...
  <Pane title="PROJECT :: PID0" meta={sync} className="project-pane pid0-profile pid0-plain verified-project">
   <div className="pid0-scroll">
    <div className="project-title"><div className="token-glyph">P0</div><div><h2>PID0</h2><p>AGENT GATEWAY FOR PONS V2 LAUNCHES AND SIGNED DEVELOPMENT RECORDS</p><span className="signal">LOCAL WORKING BUILD / CHAIN 4663</span></div></div>
    <div className="pid0-token-card">
     <div className="pid0-token-card-head"><b>$PID0 TOKEN</b><span>{pid0Launch?'CONFIRMED':'PRE-LAUNCH'}</span></div>
     <div className="pid0-token-ca"><span>CONTRACT ADDRESS / CA</span><strong>{pid0Launch?.token_address||'NOT DEPLOYED'}</strong></div>
     <div className="pid0-token-links">
      <a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">[OFFICIAL X / @pid0launchpad]</a>
      {pid0Launch?.tx_hash&&<a href={EXPLORER+'/tx/'+pid0Launch.tx_hash} target="_blank" rel="noreferrer">[PONS LAUNCH TRANSACTION]</a>}
     </div>
     <p>{pid0Launch?'The address above was loaded from a receipt-verified PONS TokenLaunched event.':'The official $PID0 contract has not been deployed. This field will update automatically after its verified PONS launch.'}</p>
    </div>
    ...
    <Section title="REGISTRY COVERAGE"><p>The middle panel reads confirmed launches stored by this PID0 Gateway. It does not claim to index every token ever launched through PONS. External PONS launches need a future historical indexer.</p><p>Price, liquidity and holders remain NOT INDEXED until real market-data sources are connected. PID0 does not manufacture these numbers.</p></Section>
