...
const fixed=[
 {id:'zunon',token:'ZUNON',ticker:'ZUNON',type:'OFFICIAL PROJECT TOKEN',status:'OFFICIAL CA PUBLISHED',agent:'ZUNON NETWORK',network:'ROBINHOOD CHAIN / 4663',contract:'0x6614f062096C1715983b4eAb86dD7E7C13E62C22',description:'$ZUNON is the official token of the ZUNON Agent launchpad and development network. The official contract address is published and the deployed ERC-20 bytecode is live on Robinhood Chain.',proof:'OFFICIAL CA PUBLISHED / CONTRACT BYTECODE VERIFIED'},
...
export default function VerifiedMarket(){
 ...
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: ZUNON" meta={Math.max(0,records.length-3)+' AGENT / 2 TEST VERIFIED / OFFICIAL PUBLISHED'} className="list-pane token-list verified-registry">
   ...
   <div className="market-summary verified-summary"><span>VERIFIED RECORDS <b>{String(Math.max(0,records.length-1)).padStart(2,'0')}</b></span><span>$ZUNON OFFICIAL CA <b>PUBLISHED</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">OFFICIAL $ZUNON CA PUBLISHED / TEST 01 + TEST 02 VERIFIED</div>
  </Pane>
  ...
  <Pane title="PROJECT :: ZUNON" meta={sync} className="project-pane zunon-profile zunon-plain verified-project">
   <div className="zunon-scroll">
    ...
    <div className="zunon-token-card">
     ...
     <p>The official $ZUNON contract address is now published. The address above resolves to deployed ERC-20 bytecode on Robinhood Chain and is linked to the chain explorer for independent verification.</p>
    </div>
    ...
    <Section title="REGISTRY COVERAGE"><p>The middle panel combines the two onchain-verified test records with confirmed Agent launches stored by the ZUNON Gateway. The official $ZUNON entry now includes its published contract address. The Registry does not claim to index every token ever launched through PONS.</p><p>Price, liquidity and holders remain NOT INDEXED until real market-data sources are connected. ZUNON does not manufacture these numbers.</p></Section>
