...
const fixed=[
 {id:'pid0',token:'PID0',ticker:'PID0',type:'OFFICIAL PROJECT TOKEN',status:'OFFICIAL CA PENDING',agent:'PID0 NETWORK',network:'ROBINHOOD CHAIN / 4663',description:'$PID0 is the official token of the PID0 Agent launchpad and development network. Its official contract address has not yet been published.',proof:'OFFICIAL CA NOT YET PUBLISHED'},
...
export default function VerifiedMarket(){
 ...
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: PID0" meta={Math.max(0,records.length-3)+' AGENT / 2 TEST VERIFIED / OFFICIAL PENDING'} className="list-pane token-list verified-registry">
   ...
   <div className="market-summary verified-summary"><span>VERIFIED RECORDS <b>{String(Math.max(0,records.length-1)).padStart(2,'0')}</b></span><span>$PID0 OFFICIAL CA <b>PENDING</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">OFFICIAL $PID0 CA AWAITING PUBLICATION / TEST 01 + TEST 02 VERIFIED</div>
  </Pane>
  ...
  <Pane title="PROJECT :: PID0" meta={sync} className="project-pane pid0-profile pid0-plain verified-project">
   <div className="pid0-scroll">
    ...
    <div className="pid0-token-card">
     <div className="pid0-token-card-head"><b>$PID0 TOKEN</b><span>CA PENDING</span></div>
     ...
     <p>The official $PID0 contract address has not yet been published. No address should be treated as official until it appears in this field and is confirmed through the official PID0 channels.</p>
    </div>
    ...
    <Section title="LAUNCH ORDER AND STATUS OF $PID0"><p>TEST 01 and TEST 02 were used to verify the Agent-to-PONS launch path. They remain visibly classified as test launches and are not the official project token.</p><p>The official $PID0 contract address is pending publication. Until the verified address appears in the official PID0 card, Registry and official channels, no external contract should be represented as $PID0.</p></Section>
    ...
    <div className="project-section final-note"><b>OFFICIAL IDENTIFIERS</b><p>Robinhood Chain mainnet / Chain ID 4663. $PID0 CA: NOT PUBLISHED. PONS V2 factory: {FACTORY}.</p><p><a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">[OFFICIAL X]</a> <a href={apiUrl('/api/v1/status')} target="_blank" rel="noreferrer">[LIVE STATUS]</a> <a href={apiUrl('/api/v1/rules')} target="_blank" rel="noreferrer">[ACTIVE RULES]</a></p></div>
   </div>
  </Pane>
 </main>
}
