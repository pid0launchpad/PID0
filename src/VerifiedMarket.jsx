...
export default function VerifiedMarket(){
 ...
 return <main className="main-panes verified-market">
  ...
  <Pane title="TOKEN REGISTRY :: PID0" meta={Math.max(0,records.length-3)+' AGENT / 2 TEST VERIFIED / OFFICIAL PENDING'} className="list-pane token-list verified-registry">
   ...
   <div className="verified-agent-list">{records.map(record=><button key={record.id} className={(selected.id===record.id?'selected ':'')+(record.id==='pid0'?'official-token-row':'')} onClick={()=>setSelectedId(record.id)}><b>{'$'+record.ticker}{record.id==='pid0'&&<i className="verified-mark" title="Official P0 token">&#10003;</i>}</b><span>{record.token}{record.id==='pid0'&&<em>OFFICIAL</em>}</span><span>{record.status}</span><span>{record.contract?short(record.contract):record.type}</span></button>)}</div>
   <div className="market-summary verified-summary"><span>VERIFIED RECORDS <b>{String(Math.max(0,records.length-1)).padStart(2,'0')}</b></span><span>$P0 OFFICIAL CA <b>PENDING</b></span><span>TEST PONS EVENTS <b>02 VERIFIED</b></span><span>SYNC STATE <b>{sync}</b></span></div>
   <div className="pane-foot">OFFICIAL $P0 CA AWAITING PUBLICATION / TEST 01 + TEST 02 VERIFIED</div>
  </Pane>
  ...
  <Pane title="PROJECT :: PID0" meta={sync} className="project-pane pid0-profile pid0-plain verified-project">
   <div className="pid0-scroll">
    ...
    <div className="pid0-token-card">
     <div className="pid0-token-card-head"><b>$P0 TOKEN</b><span>CA PENDING</span></div>
     <div className="pid0-token-ca"><span>CONTRACT ADDRESS / CA</span><strong>NOT PUBLISHED</strong></div>
     <div className="pid0-token-links">
      <a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">[OFFICIAL X / @pid0launchpad]</a>
      {pid0Token.contract&&<a href={EXPLORER+'/address/'+pid0Token.contract} target="_blank" rel="noreferrer">[TOKEN CONTRACT]</a>}
      {pid0Token.txHash&&<a href={EXPLORER+'/tx/'+pid0Token.txHash} target="_blank" rel="noreferrer">[PONS LAUNCH TRANSACTION]</a>}
     </div>
     <p>The official $P0 contract address has not yet been published. No address should be treated as official until it appears in this field and is confirmed through the official PID0 channels.</p>
    </div>
    ...
    <Section title="REGISTRY COVERAGE"><p>The middle panel combines the two onchain-verified test records with confirmed Agent launches stored by the PID0 Gateway. The official $P0 entry remains pending until its real CA is published. The Registry does not claim to index every token ever launched through PONS.</p><p>Price, liquidity and holders remain NOT INDEXED until real market-data sources are connected. PID0 does not manufacture these numbers.</p></Section>
    ...
    <Section title="LAUNCH ORDER AND STATUS OF $P0"><p>TEST 01 and TEST 02 were used to verify the Agent-to-PONS launch path. They remain visibly classified as test launches and are not the official project token.</p><p>The official $P0 contract address is pending publication. Until the verified address appears in the official P0 card, Registry and official PID0 channels, no external contract should be represented as $P0.</p></Section>
    ...
    <div className="project-section final-note"><b>OFFICIAL IDENTIFIERS</b><p>Robinhood Chain mainnet / Chain ID 4663. $P0 CA: NOT PUBLISHED. PONS V2 factory: {FACTORY}.</p><p><a href="https://x.com/pid0launchpad" target="_blank" rel="noreferrer">[OFFICIAL X]</a> <a href={apiUrl('/api/v1/status')} target="_blank" rel="noreferrer">[LIVE STATUS]</a> <a href={apiUrl('/api/v1/rules')} target="_blank" rel="noreferrer">[ACTIVE RULES]</a></p></div>
   </div>
  </Pane>
 </main>
}
